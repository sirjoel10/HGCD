...
int8_t PChar(uint16_t Ch, TextParamStruct *T){
	...
	switch(Size){
	case 0:
		LCD_setPos(X, Y, X+4+LetterSpace, Y+7);
		...
		break;
	case 1:
		LCD_setPos(X, Y, X+9+LetterSpace, Y+15);
		...
		break;
	case 2:
		LCD_setPos(X, Y, X+14+LetterSpace, Y+23);
...
/*int8_t drawRGBBitmap1(int16_t x, int16_t y,
  uint16_t *bitmap, int16_t w, int16_t h) {
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++ ) {
            writePix(x+i, y, bitmap[j * w + i]);
        }
    }
    return 0;
}
...
void drawRGBBitmap2(int16_t x, int16_t y,
  const uint16_t bitmap[], int16_t w, int16_t h) {
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++ ) {
        	WritePix(x+i, y, bitmap[j * w + i]);
        	//WritePixForBitmap(x+i, y, bitmap[j * w + i]);
        }
    }
}
