...
#ifndef FLASHCHANGE_H_
#define FLASHCHANGE_H_

void change_bank();
void change_bank2();
uint32_t ReadFlashWord(uint32_t pageNbr, uint32_t wordNbr);
uint32_t ReadReg(uint32_t Address);
uint8_t OtherBankFlashPageErase(uint8_t page_number);
uint8_t OtherBankFlashPageProgram(uint8_t *pbuf, uint32_t buf_len, uint8_t page_number);
uint32_t GetCurrentBank();
void FlashPageRewrite(uint8_t *pbuf, uint32_t buf_len, uint8_t page_num);

#define STATUS_OK 					((uint8_t)0)
#define ERROR_BUF_LEN 				((uint8_t)1)
#define ERROR_FLASH_PAGE_NUMBER 	((uint8_t)2)
#define ERROR_CRC					((uint8_t)3)

#define FLASH_LAST_PAGE		((uint8_t)255)
